...
impl Secp {
    fn public_key(private_key: &[u8]) -> Result<Vec<u8>> {
        ...
        let sk =
            secp256k1::SecretKey::from_byte_array(key).map_err(|_| HdError::InvalidPrivateKey)?;
...
impl Provider for Secp {
    ...
    fn master(&self, a: &str, seed: &[u8]) -> Result<HdNode> {
        ...
        let key_bytes: [u8; 32] = i[..32].try_into().map_err(|_| HdError::InvalidPrivateKey)?;
        let sk = secp256k1::SecretKey::from_byte_array(key_bytes)
            .map_err(|_| HdError::InvalidPrivateKey)?;
